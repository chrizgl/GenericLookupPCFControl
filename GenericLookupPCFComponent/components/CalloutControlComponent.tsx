...
class CalloutControlComponent extends React.Component<iPropsInput> {
    ...
    LoadInitialData = () => {
        ...
        if (this._entityId.length > 0) {
            this._context.webAPI
                .retrieveRecord(this._entityTypeName, this._entityId, '?$select=_' + this._tmpField.name + '_value')
                .then(
                    function success(result) {
                        const tmpLookupId = result['_' + thisRef._tmpField.name + '_value'];
                        const tmpLookupText =
                            result['_' + thisRef._tmpField.name + '_value@OData.Community.Display.V1.FormattedValue'];
    ...
    OnNewClick = () => {
        ...
        this._context.navigation
            ...
            .then(
                function (success) {
                    const lookupValue = new Array<any>();
                    lookupValue[0] = new Object();
