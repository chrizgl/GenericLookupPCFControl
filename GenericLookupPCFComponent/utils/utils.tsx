...
export abstract class StringUtils {
    public static stringifyProperties(inputProps: { [key: string]: unknown } | null | undefined): { [key: string]: string } {
        if (!inputProps) {
            return {};
        }
...
export function IsEmpty(obj: any) {
    if (obj === undefined || obj === null || obj?.toString().trim().length === 0 || obj?.toString() === '0') return true;
    else return false;
}
...
export function IsTexChanged(originalValue: any, newValue: any) {
    if (originalValue === null || originalValue === undefined) {
        if (newValue === null || newValue === undefined || newValue?.toString().trim().length === 0 || newValue?.toString() === '---')
            return false;
        else return true;
    }
    return originalValue.toString() !== newValue.toString();
